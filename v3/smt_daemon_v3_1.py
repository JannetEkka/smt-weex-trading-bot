...
def run_daemon():
    ...
    logger.info("  V3.2.74: FLOW CONTRA EXIT + RANGE REVERT + BANNER CLEANUP — (1) FLOW contra exit: close underwater positions when FLOW shows extreme opposite (taker <0.15 LONG / >7.0 SHORT), age>=VELOCITY_EXIT_MINUTES per tier (T1=75m/T2=60m/T3=50m), no BE-SL. (2) Range gate 2H override reverted 45/55→30/70 (conservative, per V3.2.69 design). (3) FLOW flip log label: shows actual position instead of always 'mid-range'. (4) Startup banner stripped to tier config + last 5 versions.")
