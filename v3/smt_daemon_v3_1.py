...
try:
    from smt_nightly_trade_v3_1 import (
        ...
        # Trading
        execute_trade,
        find_chart_based_tp_sl,  # V3.2.90: needed for opposite swap SR pre-check (was missing → NameError)
...
def check_trading_signals():
    ...
    try:
        ...
        # CONFIDENCE_EXTRA_SLOT — DISABLED V3.2.71: was bypassing 2-slot cap at 90%+.
        # At $2-3K equity, 3rd slot leaves ~$580 margin buffer = margin call risk.
        # Proper 3rd slot at $5K+ equity, not via bypass hack.
        CONFIDENCE_EXTRA_SLOT = None
        ...
        if trade_opportunities:
            ...
            for opportunity in trade_opportunities:
                ...
                if trade_type == "new" and available_slots <= 0:
                    # V3.2.71: Extra slot bypass DISABLED — hard cap is absolute.
                    logger.info(f"  SLOT BLOCKED: {pair} {signal} {confidence:.0%} — {available_slots} slots available (hard cap)")
                    continue
                    ...
                    try:
                        ...
                        if opp_pos:
                            ...
                            try:
                                ...
                                # V3.2.90: BLOCK on error, don't proceed. Closing a profitable position
                                # without verified replacement TP = guaranteed loss with nothing to show.
                                # ETH V3.2.89 bug: find_chart_based_tp_sl not imported → NameError →
                                # "proceeding cautiously" → closed +$16 SHORT → range gate blocked LONG → zero positions.
                                logger.warning(f"  [OPPOSITE] CHART SR PRE-CHECK error ({_sr_pre_err}) — BLOCKING swap (safe default: keep existing position)")
                                continue
