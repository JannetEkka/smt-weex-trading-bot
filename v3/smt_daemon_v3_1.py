            ...
            try:
                ...
                if not position.get("is_open"):
                    ...
                    try:
                        ...
                        # V3.2.41: Brief settle delay — gives WEEX time to register auto-exec close order
                        # before querying. Plan-order executions may take 1-2s to appear in orders history.
                        time.sleep(1.5)
                        # V3.2.40: Query WEEX for the close order ID generated by the TP/SL trigger execution
...
def run_daemon():
    ...
    logger.info("  TP range: [0.40%% MIN_VIABLE, per-pair ceiling] | No TP floor — entry at resistance = discard")
    logger.info("  No fallback %% — if SR walk finds nothing viable, trade is discarded")
    logger.info("  TP walk: 2H anchor → 4H anchor (if >= 0.40%%) → 48H SR walk")
    logger.info("  SL method: lowest wick in last 12H (1H grid) + last 3 4H candles | SL floor: 1.0%%")
    ...
    logger.info("  sizing_base = max(min(available_margin, balance * 2.5), 1000.0)  [free available, not equity]")
    logger.info("  base_size = sizing_base * 0.25 (conf tiers: 80-84%%=1.0x, 85-89%%=1.25x, 90%%+=1.5x)")
    logger.info("  SOL max: 30%% of sizing_base | No per-slot cap (margin guard is the limiter)")
    # --- Opposite swap ---
    ...
    logger.info("  V3.2.41: Per-pair TP ceilings (BTC/ETH=1.5%%, SOL=2.0%%, rest=1.0%%); 4H anchor; MAX_SL_PCT=1.5%%; MIN_VIABLE=0.40%%")
    logger.info("  V3.2.40: get_recent_close_order_id() — close orderId wired into all AI log uploads")
    logger.info("  V3.2.39: 90%%+ confidence opens 5th slot when all 4 full (CONFIDENCE_EXTRA_SLOT=0.90)")
