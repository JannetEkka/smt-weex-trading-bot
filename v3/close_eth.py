"""
Close ETH position manually with proper cleanup + AI log.
Auto-detects side and size from WEEX. Cancels orphan orders first.

Usage: cd v3 && python3 close_eth.py
"""
import sys, os
...
# Auto-detect ETH position (side + size)
print("Checking ETH position on WEEX...")
...
if not eth_pos:
    print("No ETH position found. Sweeping orphan orders anyway...")
...
side = eth_pos["side"]
size = float(eth_pos["size"])
entry = float(eth_pos.get("entry_price", 0))
pnl = float(eth_pos.get("unrealized_pnl", 0))
margin = float(eth_pos.get("margin", 0))

print(f"Found: ETH {side} | size={size} | entry=${entry:.2f} | UPnL=${pnl:+.2f} | margin=${margin:.2f}")
...
print(f"\n[2/3] Closing ETH {side} (size={size})...")
...
upload_ai_log_to_weex(
    stage=f"Portfolio Manager: Close {side} ETH",
    input_data={
        "symbol": symbol,
        "side": side,
        "size": size,
    ...
    explanation=f"AI Portfolio Manager closing {side} ETH (PnL: {pnl_pct:+.1f}%). Manual close to free slot and capital for higher-probability setups.",
    order_id=order_id
)
print("  AI log uploaded.")

print(f"\nDone! ETH {side} closed. Order: {order_id}")
