...
# Known step sizes for each pair (from WEEX error messages)
# This is the actual tradeable step, not size_increment
KNOWN_STEP_SIZES = {
    "cmt_btcusdt": 0.001,
    "cmt_ethusdt": 0.001,
    "cmt_solusdt": 0.1,
...
def get_contract_info(symbol: str) -> Dict:
    ...
    try:
        ...
        if isinstance(data, list) and len(data) > 0:
            ...
            # Get step size - use known value or derive from contract_val/minOrderSize
            step_size = KNOWN_STEP_SIZES.get(symbol)
            if step_size is None:
                # Try to infer from contract_val or minOrderSize
                contract_val = float(info.get("contract_val", "1"))
                min_order = float(info.get("minOrderSize", "0.001"))
                # Use the larger of contract_val or the smallest sensible step
                step_size = max(contract_val, min_order) if contract_val >= 1 else min_order
            ...
            contract_info = {
                ...
                "step_size": step_size,  # Actual trading step
                "contract_val": float(info.get("contract_val", "1")),
            }
            CONTRACT_INFO_CACHE[symbol] = contract_info
            print(f"  [CONTRACT] {symbol}: step={step_size}, min={contract_info['min_order_size']}")
            return contract_info
    except Exception as e:
        print(f"  [WEEX] Error getting contract info: {e}")
    
    # Default fallback
    return {"size_increment": "3", "tick_size": "2", "min_order_size": 0.001, "step_size": 0.001}


def round_size_to_step(size: float, symbol: str) -> float:
    """Round size DOWN to match WEEX stepSize requirement"""
    contract_info = get_contract_info(symbol)
    
    # Use the actual step_size
    step = contract_info.get("step_size", 0.001)
    
    # Round DOWN to nearest step (floor division)
    # This ensures we don't exceed available balance
    import math
    ...
    try:
        ...
        if isinstance(data, list):
            for pos in data:
                ...
                margin = float(pos.get("marginSize", 0))  # WEEX uses marginSize not margin
                ...
                # Position is truly open only if it has size > 0
                if size > 0:
                    positions.append({
                        "symbol": pos.get("symbol"),
                        "side": pos.get("side", "").upper(),  # WEEX returns uppercase
                        "size": size,
                        "entry_price": entry_price,
                        "unrealized_pnl": float(pos.get("unrealizePnl", 0)),  # WEEX uses unrealizePnl
                        "margin": margin,
                    })
        return positions
    except Exception as e:
...
def place_order(symbol: str, side: str, size: float, tp_price: float = None, sl_price: float = None) -> Dict:
    """
    Place order on WEEX with optional TP/SL
    side: "1"=OpenLong, "2"=OpenShort, "3"=CloseLong, "4"=CloseShort
    
    Uses LIMIT order slightly above/below market for instant fill when TP/SL needed.
    Uses MARKET order when no TP/SL (for closing positions).
    """
    ...
    # Determine if we need limit order (for TP/SL) or market order
    use_limit = tp_price is not None or sl_price is not None
    
    if use_limit and current_price > 0:
        ...
        # Market order (no TP/SL, used for closing)
        order = {
            "symbol": symbol,
            "client_oid": f"smt_{int(time.time()*1000)}",
            "size": str(rounded_size),
...
def place_plan_order(symbol: str, side: str, size: float, trigger_price: float) -> Dict:
    """
    Place a trigger/plan order (for TP/SL)
    side: "3"=CloseLong (for LONG position TP/SL), "4"=CloseShort (for SHORT position TP/SL)
    trigger_price: Price at which the order triggers
    
    Uses /capi/v2/order/plan_order endpoint
    """
    endpoint = "/capi/v2/order/plan_order"
    rounded_size = round_size_to_step(size, symbol)
    
    order = {
        "symbol": symbol,
        "client_oid": f"smt_plan_{int(time.time()*1000)}",
        "size": str(rounded_size),
        "type": side,
        "match_type": "1",  # Market price when triggered
        "execute_price": "0",  # Not used for market orders
    ...
    if TEST_MODE:
        print(f"  [TEST MODE] Would place plan order: {order}")
        return {"order_id": f"test_plan_{int(time.time())}", "test_mode": True}
    
    r = requests.post(f"{WEEX_BASE_URL}{endpoint}", headers=weex_headers("POST", endpoint, body), data=body, timeout=15)
    result = r.json()
    
    if result.get("order_id"):
        print(f"  [PLAN ORDER] Placed: {side} @ trigger ${trigger_price:.2f}, order_id: {result['order_id']}")
    else:
        print(f"  [PLAN ORDER] Error: {result}")
...
def execute_trade(pair_info: Dict, decision: Dict, balance: float) -> Dict:
    ...
    print(f"  [Trade] Placing {signal} @ ~${current_price:.2f}")
    ...
    if not order_id:
        return {
            "executed": False,
            "reason": f"Order failed: {result.get('msg', result)}",
            "result": result,
        }
...
def check_position_status(symbol: str) -> Dict:
    ...
    try:
        ...
        # singlePosition returns a list, not a dict
        if isinstance(data, list) and len(data) > 0:
            pos = data[0]
            size = float(pos.get("size", 0))
